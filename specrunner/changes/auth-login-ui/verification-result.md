# Verification Result — auth-login-ui — iter 1
...
| 2 | test | passed | 10.5s | 0 |
| 3 | lint | passed | 5.3s | 0 |
| 4 | build | passed | 9.2s | 0 |
...
packages/shared test:    Start at  11:10:04
packages/shared test:    Duration  295ms (transform 103ms, setup 0ms, collect 171ms, tests 21ms, environment 1ms, prepare 430ms)
...
packages/iam test:  ✓ src/user.test.ts (9 tests) 7ms
...
packages/catalog test:    Start at  11:10:05
packages/catalog test:    Duration  936ms (transform 100ms, setup 0ms, collect 185ms, tests 10ms, environment 0ms, prepare 506ms)
...
packages/crm test:    Start at  11:10:05
packages/crm test:    Duration  969ms (transform 162ms, setup 0ms, collect 381ms, tests 10ms, environment 55ms, prepare 644ms)
packages/iam test:  ✓ src/role.test.ts (7 tests) 8ms
packages/crm test: Done
packages/resource test:  ✓ src/availability.test.ts (23 tests) 9ms
packages/iam test:  ✓ src/in-memory-user-repository.test.ts (8 tests) 5ms
packages/iam test:  Test Files  3 passed (3)
packages/iam test:       Tests  24 passed (24)
packages/iam test:    Start at  11:10:05
packages/iam test:    Duration  1.03s (transform 189ms, setup 0ms, collect 451ms, tests 19ms, environment 0ms, prepare 700ms)
...
packages/resource test:    Start at  11:10:05
packages/resource test:    Duration  1.07s (transform 174ms, setup 0ms, collect 408ms, tests 23ms, environment 1ms, prepare 657ms)
...
packages/scheduling test:    Start at  11:10:06
packages/scheduling test:    Duration  295ms (transform 137ms, setup 0ms, collect 263ms, tests 23ms, environment 1ms, prepare 340ms)
packages/scheduling test: Done
packages/db test$ vitest run
packages/db test:  RUN  v2.1.9 packages/db
packages/db test:  ✓ src/ensure-schema.test.ts (3 tests) 3522ms
packages/db test:    ✓ ensureSchema > 4 テーブル（customers, resources, services, bookings）が作成される 1988ms
packages/db test:    ✓ ensureSchema > 2 回実行してもエラーが発生しない（冪等） 759ms
packages/db test:    ✓ ensureSchema > ensureSchema 後に Drizzle repo が正常に動作する（Customer save → findById） 775ms
packages/db test:  ✓ src/drizzle-resource-repository.test.ts (5 tests) 4648ms
packages/db test:    ✓ DrizzleResourceRepository > save した Resource を findById で全フィールド一致で取得できる 1932ms
packages/db test:    ✓ DrizzleResourceRepository > 未保存の id で findById すると null が返る 789ms
packages/db test:    ✓ DrizzleResourceRepository > 複数の Resource を save し、list が全件返す 683ms
packages/db test:    ✓ DrizzleResourceRepository > 同一 id で再 save すると既存データが更新される（upsert） 616ms
packages/db test:    ✓ DrizzleResourceRepository > 行 → Resource の再構成が capacity >= 1 の不変条件を createResource 経由で通す 624ms
packages/db test:  ✓ src/drizzle-booking-repository.test.ts (6 tests) 5008ms
packages/db test:    ✓ DrizzleBookingRepository > save した Booking を findById で全フィールド一致で取得できる 1955ms
packages/db test:    ✓ DrizzleBookingRepository > 未保存の id で findById すると null が返る 792ms
packages/db test:    ✓ DrizzleBookingRepository > 複数の Booking を save し、list が全件返す 552ms
packages/db test:    ✓ DrizzleBookingRepository > 同一 id で再 save すると既存データが更新される（upsert） 649ms
packages/db test:    ✓ DrizzleBookingRepository > findActiveByResource が指定 resource の active のみ返す 613ms
packages/db test:    ✓ DrizzleBookingRepository > 2026年の epoch ms (1_800_000_000_000) が欠損なく往復する 446ms
packages/db test:  ✓ src/drizzle-customer-repository.test.ts (7 tests) 5444ms
packages/db test:    ✓ DrizzleCustomerRepository > save した Customer を findById で全フィールド一致で取得できる 1952ms
packages/db test:    ✓ DrizzleCustomerRepository > 未保存の id で findById すると null が返る 770ms
packages/db test:    ✓ DrizzleCustomerRepository > 複数の Customer を save し、list が全件返す 743ms
packages/db test:    ✓ DrizzleCustomerRepository > 同一 id で再 save すると既存データが更新される（upsert） 651ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > phone のみの Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 585ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > email のみの Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 394ms
...
packages/db test:  ✓ src/drizzle-service-repository.test.ts (8 tests) 5743ms
packages/db test:    ✓ DrizzleServiceRepository > save した Service を findById で全フィールド一致で取得できる 1950ms
packages/db test:    ✓ DrizzleServiceRepository > 未保存の id で findById すると null が返る 745ms
packages/db test:    ✓ DrizzleServiceRepository > 複数の Service を save し、list が全件返す 629ms
packages/db test:    ✓ DrizzleServiceRepository > 同一 id で再 save すると既存データが更新される（upsert） 675ms
packages/db test:    ✓ DrizzleServiceRepository > duration が往復で保たれる（ofMilliseconds 経由で再構成） 597ms
packages/db test:    ✓ DrizzleServiceRepository > price が往復で保たれる（createMoney 経由で再構成） 453ms
packages/db test:    ✓ DrizzleServiceRepository > resourceKinds が往復で保たれる（非空配列） 357ms
packages/db test:    ✓ DrizzleServiceRepository > resourceKinds が空配列の場合も往復する 337ms
packages/db test:  Test Files  5 passed (5)
packages/db test:       Tests  29 passed (29)
packages/db test:    Start at  11:10:07
packages/db test:    Duration  6.28s (transform 204ms, setup 0ms, collect 1.61s, tests 24.37s, environment 1ms, prepare 270ms)
...
apps/web test:  ✓ lib/parse-service-input.test.ts (17 tests) 7ms
apps/web test:  ✓ lib/parse-resource-input.test.ts (14 tests) 4ms
apps/web test:  ✓ app/customers/actions.test.ts (4 tests) 4ms
apps/web test:  ✓ app/resources/actions.test.ts (4 tests) 7ms
apps/web test:  ✓ lib/create-booking-use-case.test.ts (6 tests) 3ms
apps/web test:  ✓ app/bookings/actions.test.ts (8 tests) 13ms
apps/web test:  ✓ lib/auth-config.test.ts (10 tests) 3ms
apps/web test:  ✓ lib/parse-booking-input.test.ts (8 tests) 6ms
apps/web test:  ✓ lib/session.test.ts (8 tests) 10ms
apps/web test:  ✓ lib/dashboard.test.ts (2 tests) 2ms
apps/web test:  ✓ app/services/actions.test.ts (3 tests) 4ms
apps/web test:  ✓ lib/parse-login-input.test.ts (7 tests) 40ms
apps/web test:  ✓ lib/route-protection.test.ts (9 tests) 2ms
apps/web test:  ✓ lib/persistence-mode.test.ts (4 tests) 1ms
apps/web test:  ✓ lib/booking-transitions.test.ts (10 tests) 6ms
apps/web test:  ✓ lib/composition-root.test.ts (5 tests) 41ms
apps/web test:  ✓ lib/password.test.ts (6 tests) 245ms
apps/web test:  ✓ lib/authenticate.test.ts (4 tests) 278ms
apps/web test:  Test Files  19 passed (19)
apps/web test:       Tests  140 passed (140)
apps/web test:    Start at  11:10:13
apps/web test:    Duration  1.14s (transform 517ms, setup 0ms, collect 1.56s, tests 681ms, environment 2ms, prepare 1.31s)
apps/web test: Done
...
apps/web build:  ✓ Compiled successfully in 1430ms
...
apps/web build: ┌ ƒ /                                      164 B         106 kB
apps/web build: ├ ƒ /_not-found                            993 B         103 kB
apps/web build: ├ ƒ /bookings                            1.41 kB         104 kB
apps/web build: ├ ƒ /customers                             968 B         103 kB
apps/web build: ├ ƒ /login                                 861 B         103 kB
apps/web build: ├ ƒ /resources                             927 B         103 kB
apps/web build: └ ƒ /services                            1.03 kB         104 kB
...
apps/web build: ƒ Middleware                             34.6 kB
