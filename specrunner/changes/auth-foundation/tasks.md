...
- [ ] `apps/web/package.json` の `dependencies` に `"@koma/iam": "workspace:*"` を追加する
- [ ] `pnpm install` を実行してロックファイルを更新する
- [ ] bcrypt / argon2 / jose / iron-session が `dependencies` / `devDependencies` に含まれていないことを確認する
...
- [ ] `apps/web/lib/password.ts` を新規作成する
- [ ] `hashPassword(plain: string): Promise<string>` を実装する
...
- [ ] `verifyPassword(plain: string, stored: string): Promise<boolean>` を実装する
  - stored から salt と hash を分離
  - 同じ salt で plain をハッシュし、`crypto.timingSafeEqual` で定数時間比較
  - 形式不正時は `false` を返す
- [ ] `apps/web/lib/password.test.ts` を新規作成する
...
- [ ] `apps/web/lib/session.ts` を新規作成する
- [ ] `SessionPayload` 型を定義: `{ userId: string; role: Role; exp: number }`（`Role` は `@koma/iam` から import）
- [ ] `signSession(payload: SessionPayload, secret: string): Promise<string>` を実装する
...
- [ ] `verifySession(token: string, secret: string, now?: number): Promise<SessionPayload | null>` を実装する
...
- [ ] `apps/web/lib/session.test.ts` を新規作成する
...
- [ ] `apps/web/lib/auth-config.ts` を新規作成する
- [ ] `AuthConfig` 型を定義: `{ sessionSecret: string; adminEmail: string; adminPassword: string }`
- [ ] `resolveAuthConfig(env: Record<string, string | undefined>): AuthConfig` を実装する
...
- [ ] `apps/web/lib/auth-config.test.ts` を新規作成する
...
- [ ] `apps/web/lib/composition-root.ts` を編集する
...
- [ ] `apps/web/lib/composition-root.test.ts` を新規作成する
...
- [ ] `apps/web/lib/authenticate.ts` を新規作成する
- [ ] `authenticate(repo: UserRepository, email: string, password: string): Promise<User | null>` を実装する
  - `repo.findByEmail(email)` → 見つからなければ `null`
  - `verifyPassword(password, user.passwordHash)` → `false` なら `null`
  - 成功時は `User` を返す
- [ ] `apps/web/lib/authenticate.test.ts` を新規作成する
...
- [ ] 既存テスト（`persistence-mode.test.ts` / `create-booking-use-case.test.ts` 等）が壊れていない
