...
- [x] `packages/shared/` ディレクトリを作成する
- [x] `packages/shared/package.json` を作成する:
  - `name`: `"@koma/shared"`
  - `private`: `true`
  - `type`: `"module"`
...
- [x] `packages/shared/tsconfig.json` を作成する:
  - `target`: `"ES2022"`、`module`: `"ES2022"`（Node 20+ 対応）
  - `moduleResolution`: `"bundler"`
  - `strict`: `true`、`noEmit`: `true`
  - `include`: `["src"]`
- [x] `packages/shared/vitest.config.ts` を作成する（デフォルト設定、`include` は `src/**/*.test.ts`）
- [x] `packages/shared/eslint.config.js` を作成する（`@eslint/js` + `typescript-eslint` の推奨構成）
- [x] `packages/shared/src/index.ts` を空ファイルとして作成する（後続タスクで re-export を追加）
...
- [x] `packages/shared/src/id.ts` を作成する:
...
- [x] `packages/shared/src/id.test.ts` を作成する:
...
- [x] `packages/shared/src/index.ts` に `Id`, `createId`, `parseId`, `isEqualId` を re-export する
...
- [x] `packages/shared/src/money.ts` を作成する:
...
- [x] `packages/shared/src/money.test.ts` を作成する:
  - 整数 amount での生成を検証
...
- [x] `packages/shared/src/index.ts` に `Currency`, `Money`, `createMoney`, `addMoney`, `subtractMoney`, `compareMoney`, `isEqualMoney` を re-export する
...
- [x] `packages/shared/src/duration.ts` を作成する:
...
- [x] `packages/shared/src/duration.test.ts` を作成する:
...
- [x] `packages/shared/src/index.ts` に `Duration`, `ofMilliseconds`, `ofMinutes`, `ofHours`, `toMilliseconds`, `toMinutes`, `isEqualDuration` を re-export する
...
- [x] `packages/shared/src/time-range.ts` を作成する:
...
- [x] `packages/shared/src/time-range.test.ts` を作成する:
...
- [x] `packages/shared/src/index.ts` に `TimeRange`, `createTimeRange`, `overlaps`, `contains`, `duration` (名前衝突を避けるため `timeRangeDuration` 等のエイリアスを検討), `isEqualTimeRange` を re-export する
...
- [x] `packages/shared/src/index.ts` の re-export が全値オブジェクトの public 型・関数を網羅していることを確認する
