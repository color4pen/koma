...
- [x] `packages/scheduling/package.json` を作成する。`@koma/resource` の `package.json` をベースに以下を変更:
...
- [x] `packages/scheduling/tsconfig.json` を作成する（resource と同一内容: `ES2022` / `bundler` / `strict` / `noEmit`）
- [x] `packages/scheduling/vitest.config.ts` を作成する（resource と同一内容）
- [x] `packages/scheduling/eslint.config.js` を作成する（resource と同一内容）
- [x] `packages/scheduling/src/index.ts` を空のバレルファイルとして作成する
...
- [x] `BookingStatus` 型を定義する: `'pending' | 'confirmed' | 'cancelled' | 'completed' | 'no-show'`
- [x] `ALLOWED_TRANSITIONS` を `Map<BookingStatus, ReadonlySet<BookingStatus>>` として定義する:
  - `pending → { confirmed, cancelled }`
  - `confirmed → { cancelled, completed, no-show }`
  - `cancelled → ∅`
  - `completed → ∅`
  - `no-show → ∅`
- [x] `isActive(status: BookingStatus): boolean` — `pending` | `confirmed` で `true`
- [x] `isTerminal(status: BookingStatus): boolean` — `cancelled` | `completed` | `no-show` で `true`
- [x] `src/index.ts` から `BookingStatus` 型、`isActive`、`isTerminal` を export する
...
- [x] `isActive` のテスト: 5 値全てに対して期待値を検証する
- [x] `isTerminal` のテスト: 5 値全てに対して期待値を検証する
- [x] `isActive` と `isTerminal` が相互排他で全状態をカバーすることを検証する
...
- [x] `CustomFieldValue` 型を定義する: `string | number | boolean`
- [x] `Booking` 型を定義する（全フィールド `readonly`）:
  - `id: Id<'Booking'>`
...
- [x] `restoreBooking` ファクトリ関数を実装する（永続化復元・テスト用）:
  - 引数: `Booking` の全フィールドを必須で受け取る（`status` を含む）
  - 返却オブジェクトを `Object.freeze` する
- [x] `transitionBooking(booking: Booking, to: BookingStatus): Booking` を実装する:
  - `ALLOWED_TRANSITIONS` を参照し、許可遷移なら `status` を変えた新しい frozen `Booking` を返す
  - 不正遷移（同一状態への遷移を含む）は `Error` を throw する
  - 元の `Booking` は変更しない
- [x] `src/index.ts` から `Booking` 型、`CustomFieldValue` 型、`createBooking`、`restoreBooking`、`transitionBooking` を export する
...
- [x] `createBooking` のテスト:
  - 必須フィールドのみで構築でき、`status` が `'pending'` である
  - `id` 省略時に自動生成される
  - `customFields` 省略時に空オブジェクトが設定される
  - 返却値が frozen である
- [x] `restoreBooking` のテスト:
  - 任意の `status` で復元できる
  - 返却値が frozen である
- [x] `transitionBooking` のテスト:
...
- [x] `canAccommodate(existingActive: Booking[], slot: TimeRange, capacity: number): boolean` を実装する:
...
- [x] `@koma/shared` の `overlaps` を利用して重なり判定する
- [x] `src/index.ts` から `canAccommodate` を export する
...
- [x] `createInMemoryBookingRepository` ファクトリ関数を実装する:
...
- [x] `src/index.ts` から `createInMemoryBookingRepository` を export する
...
- [x] 以下を全て `src/index.ts` から export する:
...
- [x] `pnpm -F @koma/scheduling run check-types` で export 整合性を確認する
