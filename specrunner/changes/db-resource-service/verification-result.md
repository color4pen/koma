# Verification Result — db-resource-service — iter 1
...
packages/shared test:    Start at  02:10:46
packages/shared test:    Duration  255ms (transform 120ms, setup 0ms, collect 191ms, tests 18ms, environment 1ms, prepare 360ms)
...
packages/crm test:  ✓ src/contact-info.test.ts (7 tests) 5ms
...
packages/catalog test:    Start at  02:10:46
packages/catalog test:    Duration  827ms (transform 102ms, setup 0ms, collect 201ms, tests 8ms, environment 0ms, prepare 439ms)
packages/crm test:  Test Files  3 passed (3)
packages/crm test:       Tests  26 passed (26)
packages/crm test:    Start at  02:10:46
packages/crm test:    Duration  845ms (transform 136ms, setup 0ms, collect 418ms, tests 12ms, environment 1ms, prepare 325ms)
...
packages/resource test:    Start at  02:10:46
packages/resource test:    Duration  879ms (transform 120ms, setup 0ms, collect 360ms, tests 21ms, environment 1ms, prepare 622ms)
...
packages/scheduling test:    Start at  02:10:46
packages/scheduling test:    Duration  911ms (transform 235ms, setup 0ms, collect 673ms, tests 18ms, environment 0ms, prepare 1.03s)
...
apps/web test:  ✓ app/bookings/actions.test.ts (4 tests) 11ms
...
apps/web test:    Start at  02:10:47
apps/web test:    Duration  804ms (transform 370ms, setup 0ms, collect 1.18s, tests 53ms, environment 2ms, prepare 818ms)
apps/web test: Done
packages/db test:  ✓ src/drizzle-resource-repository.test.ts (5 tests) 2524ms
packages/db test:    ✓ DrizzleResourceRepository > save した Resource を findById で全フィールド一致で取得できる 958ms
packages/db test:    ✓ DrizzleResourceRepository > 未保存の id で findById すると null が返る 401ms
packages/db test:    ✓ DrizzleResourceRepository > 複数の Resource を save し、list が全件返す 408ms
packages/db test:    ✓ DrizzleResourceRepository > 同一 id で再 save すると既存データが更新される（upsert） 379ms
packages/db test:    ✓ DrizzleResourceRepository > 行 → Resource の再構成が capacity >= 1 の不変条件を createResource 経由で通す 377ms
packages/db test:  ✓ src/drizzle-customer-repository.test.ts (7 tests) 3112ms
packages/db test:    ✓ DrizzleCustomerRepository > save した Customer を findById で全フィールド一致で取得できる 899ms
packages/db test:    ✓ DrizzleCustomerRepository > 未保存の id で findById すると null が返る 381ms
packages/db test:    ✓ DrizzleCustomerRepository > 複数の Customer を save し、list が全件返す 371ms
packages/db test:    ✓ DrizzleCustomerRepository > 同一 id で再 save すると既存データが更新される（upsert） 363ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > phone のみの Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 399ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > email のみの Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 356ms
...
packages/db test:  ✓ src/drizzle-service-repository.test.ts (8 tests) 3515ms
packages/db test:    ✓ DrizzleServiceRepository > save した Service を findById で全フィールド一致で取得できる 935ms
packages/db test:    ✓ DrizzleServiceRepository > 未保存の id で findById すると null が返る 416ms
packages/db test:    ✓ DrizzleServiceRepository > 複数の Service を save し、list が全件返す 397ms
packages/db test:    ✓ DrizzleServiceRepository > 同一 id で再 save すると既存データが更新される（upsert） 370ms
packages/db test:    ✓ DrizzleServiceRepository > duration が往復で保たれる（ofMilliseconds 経由で再構成） 386ms
packages/db test:    ✓ DrizzleServiceRepository > price が往復で保たれる（createMoney 経由で再構成） 348ms
...
packages/db test:  Test Files  3 passed (3)
packages/db test:       Tests  20 passed (20)
packages/db test:    Start at  02:10:47
packages/db test:    Duration  4.38s (transform 158ms, setup 0ms, collect 1.50s, tests 9.15s, environment 0ms, prepare 321ms)
...
apps/web build: ├ ○ /customers                             968 B         103 kB
