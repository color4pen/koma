...
| 2 | test | passed | 7.9s | 0 |
| 3 | lint | passed | 4.2s | 0 |
| 4 | build | passed | 8.8s | 0 |
...
packages/shared test:    Start at  04:20:49
packages/shared test:    Duration  274ms (transform 118ms, setup 0ms, collect 184ms, tests 22ms, environment 1ms, prepare 375ms)
...
packages/resource test:  ✓ src/daily-time-range.test.ts (17 tests) 4ms
...
packages/catalog test:    Start at  04:20:50
packages/catalog test:    Duration  834ms (transform 82ms, setup 0ms, collect 157ms, tests 9ms, environment 0ms, prepare 418ms)
...
packages/resource test:  ✓ src/resource.test.ts (11 tests) 5ms
packages/crm test:  ✓ src/customer.test.ts (13 tests) 3ms
packages/crm test:  ✓ src/in-memory-customer-repository.test.ts (6 tests) 7ms
packages/scheduling test:  Test Files  5 passed (5)
packages/scheduling test:       Tests  66 passed (66)
packages/scheduling test:    Start at  04:20:50
packages/scheduling test:    Duration  918ms (transform 431ms, setup 0ms, collect 1.01s, tests 20ms, environment 1ms, prepare 756ms)
packages/resource test:  Test Files  4 passed (4)
packages/resource test:       Tests  57 passed (57)
packages/resource test:    Start at  04:20:50
packages/resource test:    Duration  923ms (transform 123ms, setup 0ms, collect 257ms, tests 20ms, environment 0ms, prepare 510ms)
packages/crm test:  Test Files  3 passed (3)
packages/crm test:       Tests  26 passed (26)
packages/crm test:    Start at  04:20:50
packages/crm test:    Duration  935ms (transform 260ms, setup 0ms, collect 513ms, tests 12ms, environment 0ms, prepare 450ms)
...
packages/db test:  ✓ src/ensure-schema.test.ts (3 tests) 2618ms
packages/db test:    ✓ ensureSchema > 4 テーブル（customers, resources, services, bookings）が作成される 1431ms
packages/db test:    ✓ ensureSchema > 2 回実行してもエラーが発生しない（冪等） 605ms
packages/db test:    ✓ ensureSchema > ensureSchema 後に Drizzle repo が正常に動作する（Customer save → findById） 581ms
packages/db test:  ✓ src/drizzle-resource-repository.test.ts (5 tests) 3433ms
packages/db test:    ✓ DrizzleResourceRepository > save した Resource を findById で全フィールド一致で取得できる 1392ms
packages/db test:    ✓ DrizzleResourceRepository > 未保存の id で findById すると null が返る 565ms
packages/db test:    ✓ DrizzleResourceRepository > 複数の Resource を save し、list が全件返す 554ms
packages/db test:    ✓ DrizzleResourceRepository > 同一 id で再 save すると既存データが更新される（upsert） 488ms
packages/db test:    ✓ DrizzleResourceRepository > 行 → Resource の再構成が capacity >= 1 の不変条件を createResource 経由で通す 434ms
packages/db test:  ✓ src/drizzle-booking-repository.test.ts (6 tests) 3896ms
packages/db test:    ✓ DrizzleBookingRepository > save した Booking を findById で全フィールド一致で取得できる 1520ms
packages/db test:    ✓ DrizzleBookingRepository > 未保存の id で findById すると null が返る 549ms
packages/db test:    ✓ DrizzleBookingRepository > 複数の Booking を save し、list が全件返す 623ms
packages/db test:    ✓ DrizzleBookingRepository > 同一 id で再 save すると既存データが更新される（upsert） 437ms
packages/db test:    ✓ DrizzleBookingRepository > findActiveByResource が指定 resource の active のみ返す 405ms
packages/db test:    ✓ DrizzleBookingRepository > 2026年の epoch ms (1_800_000_000_000) が欠損なく往復する 361ms
packages/db test:  ✓ src/drizzle-customer-repository.test.ts (7 tests) 4111ms
packages/db test:    ✓ DrizzleCustomerRepository > save した Customer を findById で全フィールド一致で取得できる 1376ms
packages/db test:    ✓ DrizzleCustomerRepository > 未保存の id で findById すると null が返る 580ms
packages/db test:    ✓ DrizzleCustomerRepository > 複数の Customer を save し、list が全件返す 484ms
packages/db test:    ✓ DrizzleCustomerRepository > 同一 id で再 save すると既存データが更新される（upsert） 511ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > phone のみの Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 422ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > email のみの Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 389ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > phone と email の両方を持つ Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 349ms
packages/db test:  ✓ src/drizzle-service-repository.test.ts (8 tests) 4511ms
packages/db test:    ✓ DrizzleServiceRepository > save した Service を findById で全フィールド一致で取得できる 1415ms
packages/db test:    ✓ DrizzleServiceRepository > 未保存の id で findById すると null が返る 647ms
packages/db test:    ✓ DrizzleServiceRepository > 複数の Service を save し、list が全件返す 530ms
packages/db test:    ✓ DrizzleServiceRepository > 同一 id で再 save すると既存データが更新される（upsert） 469ms
packages/db test:    ✓ DrizzleServiceRepository > duration が往復で保たれる（ofMilliseconds 経由で再構成） 399ms
packages/db test:    ✓ DrizzleServiceRepository > price が往復で保たれる（createMoney 経由で再構成） 373ms
packages/db test:    ✓ DrizzleServiceRepository > resourceKinds が往復で保たれる（非空配列） 356ms
packages/db test:    ✓ DrizzleServiceRepository > resourceKinds が空配列の場合も往復する 320ms
packages/db test:  Test Files  5 passed (5)
packages/db test:       Tests  29 passed (29)
packages/db test:    Start at  04:20:51
packages/db test:    Duration  4.98s (transform 186ms, setup 0ms, collect 1.40s, tests 18.57s, environment 1ms, prepare 238ms)
...
apps/web test:  ✓ lib/parse-customer-input.test.ts (11 tests) 6ms
apps/web test:  ✓ app/customers/actions.test.ts (4 tests) 4ms
apps/web test:  ✓ app/resources/actions.test.ts (4 tests) 4ms
apps/web test:  ✓ lib/create-booking-use-case.test.ts (6 tests) 6ms
apps/web test:  ✓ app/bookings/actions.test.ts (8 tests) 23ms
apps/web test:  ✓ lib/persistence-mode.test.ts (4 tests) 2ms
...
apps/web test:    Start at  04:20:56
apps/web test:    Duration  597ms (transform 404ms, setup 0ms, collect 998ms, tests 68ms, environment 1ms, prepare 732ms)
apps/web test: Done
...
apps/web build:  ✓ Compiled successfully in 1370ms
