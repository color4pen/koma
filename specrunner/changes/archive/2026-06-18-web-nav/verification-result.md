# Verification Result — web-nav — iter 1
...
|---|-------|--------|----------|-----------|
| 1 | typecheck | passed | 2.5s | 0 |
| 2 | test | passed | 7.1s | 0 |
| 3 | lint | passed | 4.3s | 0 |
...
packages/shared test:    Start at  03:38:23
packages/shared test:    Duration  245ms (transform 123ms, setup 0ms, collect 206ms, tests 18ms, environment 1ms, prepare 344ms)
...
packages/resource test:  ✓ src/in-memory-resource-repository.test.ts (6 tests) 7ms
packages/resource test:  ✓ src/resource.test.ts (11 tests) 9ms
packages/crm test:  Test Files  3 passed (3)
packages/crm test:       Tests  26 passed (26)
packages/crm test:    Start at  03:38:23
packages/crm test:    Duration  829ms (transform 129ms, setup 0ms, collect 372ms, tests 12ms, environment 0ms, prepare 626ms)
...
packages/catalog test:    Start at  03:38:23
packages/catalog test:    Duration  854ms (transform 76ms, setup 0ms, collect 189ms, tests 10ms, environment 0ms, prepare 489ms)
...
packages/scheduling test:  ✓ src/in-memory-booking-repository.test.ts (11 tests) 6ms
packages/resource test:  Test Files  4 passed (4)
packages/resource test:       Tests  57 passed (57)
packages/resource test:    Start at  03:38:23
packages/resource test:    Duration  912ms (transform 107ms, setup 0ms, collect 221ms, tests 27ms, environment 1ms, prepare 807ms)
packages/crm test: Done
packages/scheduling test:  Test Files  5 passed (5)
packages/scheduling test:       Tests  66 passed (66)
packages/scheduling test:    Start at  03:38:23
packages/scheduling test:    Duration  928ms (transform 260ms, setup 0ms, collect 766ms, tests 25ms, environment 1ms, prepare 1.01s)
...
apps/web test:  ✓ lib/parse-service-input.test.ts (17 tests) 4ms
...
apps/web test:  ✓ app/bookings/actions.test.ts (8 tests) 9ms
...
apps/web test:  Test Files  11 passed (11)
apps/web test:       Tests  87 passed (87)
apps/web test:    Start at  03:38:24
apps/web test:    Duration  908ms (transform 380ms, setup 0ms, collect 1.29s, tests 67ms, environment 5ms, prepare 1.07s)
apps/web test: Done
packages/db test:  ✓ src/drizzle-resource-repository.test.ts (5 tests) 3036ms
packages/db test:    ✓ DrizzleResourceRepository > save した Resource を findById で全フィールド一致で取得できる 1118ms
packages/db test:    ✓ DrizzleResourceRepository > 未保存の id で findById すると null が返る 574ms
packages/db test:    ✓ DrizzleResourceRepository > 複数の Resource を save し、list が全件返す 449ms
packages/db test:    ✓ DrizzleResourceRepository > 同一 id で再 save すると既存データが更新される（upsert） 466ms
packages/db test:    ✓ DrizzleResourceRepository > 行 → Resource の再構成が capacity >= 1 の不変条件を createResource 経由で通す 428ms
packages/db test:  ✓ src/drizzle-booking-repository.test.ts (6 tests) 3428ms
packages/db test:    ✓ DrizzleBookingRepository > save した Booking を findById で全フィールド一致で取得できる 1150ms
packages/db test:    ✓ DrizzleBookingRepository > 未保存の id で findById すると null が返る 559ms
packages/db test:    ✓ DrizzleBookingRepository > 複数の Booking を save し、list が全件返す 437ms
packages/db test:    ✓ DrizzleBookingRepository > 同一 id で再 save すると既存データが更新される（upsert） 443ms
packages/db test:    ✓ DrizzleBookingRepository > findActiveByResource が指定 resource の active のみ返す 445ms
packages/db test:    ✓ DrizzleBookingRepository > 2026年の epoch ms (1_800_000_000_000) が欠損なく往復する 392ms
packages/db test:  ✓ src/drizzle-customer-repository.test.ts (7 tests) 3734ms
packages/db test:    ✓ DrizzleCustomerRepository > save した Customer を findById で全フィールド一致で取得できる 1133ms
packages/db test:    ✓ DrizzleCustomerRepository > 未保存の id で findById すると null が返る 540ms
packages/db test:    ✓ DrizzleCustomerRepository > 複数の Customer を save し、list が全件返す 447ms
packages/db test:    ✓ DrizzleCustomerRepository > 同一 id で再 save すると既存データが更新される（upsert） 452ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > phone のみの Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 419ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > email のみの Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 395ms
packages/db test:    ✓ DrizzleCustomerRepository > 行 → Customer の再構成が集約不変条件を満たす > phone と email の両方を持つ Customer を save → findById で再構成すると ContactInfo が不変条件を満たす 346ms
packages/db test:  ✓ src/drizzle-service-repository.test.ts (8 tests) 4084ms
packages/db test:    ✓ DrizzleServiceRepository > save した Service を findById で全フィールド一致で取得できる 1124ms
packages/db test:    ✓ DrizzleServiceRepository > 未保存の id で findById すると null が返る 568ms
packages/db test:    ✓ DrizzleServiceRepository > 複数の Service を save し、list が全件返す 412ms
packages/db test:    ✓ DrizzleServiceRepository > 同一 id で再 save すると既存データが更新される（upsert） 464ms
packages/db test:    ✓ DrizzleServiceRepository > duration が往復で保たれる（ofMilliseconds 経由で再構成） 425ms
packages/db test:    ✓ DrizzleServiceRepository > price が往復で保たれる（createMoney 経由で再構成） 402ms
packages/db test:    ✓ DrizzleServiceRepository > resourceKinds が往復で保たれる（非空配列） 355ms
packages/db test:    ✓ DrizzleServiceRepository > resourceKinds が空配列の場合も往復する 332ms
packages/db test:  Test Files  4 passed (4)
packages/db test:       Tests  26 passed (26)
packages/db test:    Start at  03:38:24
packages/db test:    Duration  5.03s (transform 231ms, setup 0ms, collect 2.22s, tests 14.28s, environment 1ms, prepare 545ms)
...
apps/web build:  ✓ Compiled successfully in 837ms
...
apps/web build: ├ ○ /customers                             967 B         103 kB
