...
- [x] `packages/db/package.json` の `dependencies` に `"@koma/scheduling": "workspace:*"` を追加する
...
- [x] `packages/db/src/schema/booking.ts` を新規作成する
- [x] `pgTable('bookings', ...)` で以下のカラムを定義する:
...
- [x] `BookingRow` 型を export する（`typeof bookings.$inferSelect`）
- [x] import は `drizzle-orm/pg-core` から `pgTable`, `text`, `bigint`, `jsonb` を使用する
...
- [x] `packages/db/src/drizzle-booking-repository.ts` を新規作成する
- [x] `createDrizzleBookingRepository(db: DrizzleClient): BookingRepository` ファクトリ関数を export する
- [x] `rowToBooking` ヘルパ関数を実装する:
...
- [x] `save` メソッド: `insert(bookings).values(...).onConflictDoUpdate({ target: bookings.id, set: ... })` で upsert
  - `slot.start` → `start_millis`、`slot.end` → `end_millis` としてマッピング
  - `customFields` は `{ ...booking.customFields }` でスプレッドして frozen を解除
- [x] `findById` メソッド: `select().from(bookings).where(eq(bookings.id, id))` → 先頭行を `rowToBooking` で変換、無ければ `null`
- [x] `list` メソッド: `select().from(bookings)` → `rows.map(rowToBooking)`
- [x] `findActiveByResource` メソッド: `select().from(bookings).where(and(eq(bookings.resource_id, resourceId), inArray(bookings.status, ['pending', 'confirmed'])))` → `rows.map(rowToBooking)`
- [x] import 元: `drizzle-orm` から `eq`, `and`, `inArray`。`@koma/scheduling` から `BookingRepository`, `Booking`, `BookingStatus`, `CustomFieldValue`, `restoreBooking`。`@koma/shared` から `parseId`, `createTimeRange`
...
- [x] `packages/db/src/drizzle-booking-repository.test.ts` を新規作成する
- [x] テストファイルの先頭で pglite セットアップを定義する:
...
- [x] テストデータ生成のヘルパまたは共通変数を用意する（`createId`, `createTimeRange`, `createBooking` / `restoreBooking` を使用）
- [x] **テスト 1: save → findById で slot と status が往復する**
...
- [x] **テスト 6: bigint 往復（大きな epoch ms 値）**
