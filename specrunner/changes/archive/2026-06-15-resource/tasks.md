...
- [x] `packages/resource/package.json` を作成する。`@koma/crm` の `package.json` をベースに以下を変更:
  - `name`: `@koma/resource`
...
- [x] `packages/resource/tsconfig.json` を作成する（crm と同一内容）
- [x] `packages/resource/vitest.config.ts` を作成する（crm と同一内容）
- [x] `packages/resource/eslint.config.js` を作成する（crm と同一内容）
- [x] `packages/resource/src/index.ts` を空のバレルファイルとして作成する
...
- [x] `Resource` 型を定義する: `id: Id<'Resource'>`, `name: string`, `kind: string`, `capacity: number`（全フィールド `readonly`）
- [x] `createResource` ファクトリ関数を実装する:
...
- [x] `ResourceRepository` 型（interface）を定義する:
  - `save(resource: Resource): Promise<void>`
  - `findById(id: Id<'Resource'>): Promise<Resource | null>`
  - `list(): Promise<Resource[]>`
- [x] `src/index.ts` から `ResourceRepository` 型を export する
...
- [x] `createInMemoryResourceRepository` ファクトリ関数を実装する:
...
- [x] `src/index.ts` から `createInMemoryResourceRepository` を export する
