...
| **domain** | 予約 / 顧客 / 通知の業務ロジック・判定・集約。業種中立 | `packages/scheduling`, `packages/crm`, `packages/notification` |
...
| from \ to | web | scheduling | crm | notification | shared | db | ext¹ |
|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| **web（delivery）** | — | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| **scheduling** | ✗ | — | ✗ | ✗ | ✓ | ✗ | ✗ |
| **crm** | ✗ | ✗ | — | ✗ | ✓ | ✗ | ✗ |
...
- ¹ **ext** = `next` / `react` / `drizzle-orm` / `zod` 等の外部フレームワーク。許可は **web**（`next`/`react`/`zod/mini`）と **db**（`drizzle-orm`）のみ。
- ² db → domain は **port interface / 型のみ**参照可（Repository port を実装するため）。domain の実装・ユースケースは import しない。
- ³ db のみ `drizzle-orm` を import してよい。
- **兄弟 domain 非依存**: scheduling / crm / notification は相互に import しない（✗）。連携は shared のイベント契約か web の composition 経由（B-5）。
...
| # | invariant | なぜ |
|---|---|---|
| **B-1** | domain（scheduling / crm / notification）と shared は `next` / `react` を import しない | ドメインを配信・UI から独立に保つ。配信の差し替え（別フレームワーク・別チャネル）の影響を web に閉じる |
...
| **B-5** | domain パッケージは兄弟 domain パッケージ（scheduling ↔ crm ↔ notification）を直接 import しない。連携は shared のイベント契約（`dynamic-model.md`）または web の composition 経由 | モジュールを独立に保ち、業種固有の差し込みでコア間が密結合しないようにする |
